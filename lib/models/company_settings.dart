...
class CompanySettings {
  ...
  // ---- المزامنة ونوع المستخدم (مندوب ⇄ مدير) ----
  String appRole; // 'rep' (مندوب) أو 'manager' (مدير) — نوع المستخدم لهذا الجهاز
  String repCode; // رقم/رمز المندوب — يُستخدم لمطابقة ملفات المزامنة
  String deviceName; // اسم الجهاز (يظهر بسجلات المزامنة لدى المدير)
  int lastImportedUpdateNumber; // رقم آخر تحديث تم استيراده من المدير
  ...
  CompanySettings({
    ...
    this.repCode = '',
    this.deviceName = '',
    this.lastImportedUpdateNumber = 0,
  ...
  bool get isManager => appRole == 'manager';
  ...
  Map<String, dynamic> toJson() => {
        ...
        'lastImportedUpdateNumber': lastImportedUpdateNumber,
      };
  ...
  factory CompanySettings.fromJson(Map<String, dynamic> m) => CompanySettings(
        ...
        appRole: m['appRole'] as String? ?? 'rep',
        repCode: m['repCode'] as String? ?? '',
        deviceName: m['deviceName'] as String? ?? '',
        lastImportedUpdateNumber:
            (m['lastImportedUpdateNumber'] as num?)?.toInt() ?? 0,
      );
}

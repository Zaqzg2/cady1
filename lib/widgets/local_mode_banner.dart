...
class LocalModeBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String? text;
    if (!settings.hasAnyOcrKey) {
      text = 'وضع محلي';
    } else {
      // "معطوب فعليًا" فقط إن وُجد مفتاح واختبار سابق يؤكد الفشل — مفتاح
      // "لم يُختبر بعد" لا يُعتبر عطلًا (نفس منطق النسخة أحادية المحرك سابقًا،
      // معمَّم الآن على محركين مستقلَّين — قسم ٣٧: بلا خلط بينهما).
      final mistralBroken = settings.hasMistralKey &&
          settings.mistralHealthResult != null &&
          settings.mistralHealthResult!.status != MistralConnectionStatus.valid;
      final mistralFine = !settings.hasMistralKey ||
          settings.mistralHealthResult == null ||
          settings.mistralHealthResult!.status == MistralConnectionStatus.valid;
      final ocrSpaceBroken = settings.hasOcrSpaceKey &&
          settings.ocrSpaceHealthResult != null &&
          settings.ocrSpaceHealthResult!.status != OcrSpaceConnectionStatus.valid;
      final ocrSpaceFine = !settings.hasOcrSpaceKey ||
          settings.ocrSpaceHealthResult == null ||
          settings.ocrSpaceHealthResult!.status == OcrSpaceConnectionStatus.valid;

      final allConfiguredEnginesBroken = (mistralBroken || ocrSpaceBroken) && !mistralFine && !ocrSpaceFine;
      text = allConfiguredEnginesBroken ? 'AI غير متاح — العمل المحلي مستمر' : null;
    }
